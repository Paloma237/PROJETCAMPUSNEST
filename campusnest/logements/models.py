"""
campusnest/logements/models.py
Ajout du modèle DateVisiteChambre : le propriétaire définit
les créneaux de visite disponibles pour chaque chambre.
"""
...
class Chambre(models.Model):
    ...
    est_actif         = models.BooleanField("Visible sur le site", default=True)
    motif_suppression = models.TextField(
        "Raison de la désactivation/suppression", blank=True,
        help_text="Expliquez ici au propriétaire pourquoi sa chambre n'est plus visible."
    )
    ...
    def __str__(self):
        return f"{self.description} ({self.superficie}m²) — {self.cite.nom}"
    ...
    def dates_visites_disponibles(self):
        """Retourne les dates de visite futures encore disponibles."""
        return self.dates_visite.filter(date__gte=timezone.now().date()).order_by("date")


class DateVisiteChambre(models.Model):
    """
    Créneau de visite proposé par le propriétaire pour une chambre.
    Le client choisit parmi ces dates lors de sa réservation.
    """
    chambre = models.ForeignKey(
        Chambre,
        on_delete=models.CASCADE,
        related_name="dates_visite",
        verbose_name="Chambre",
    )
    date = models.DateField("Date de visite disponible")
    ...
    def __str__(self):
        return f"{self.chambre} — {self.date.strftime('%d/%m/%Y')}"
